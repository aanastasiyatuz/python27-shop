...
class Billing(models.Model):
    user = models.OneToOneField(User, related_name='billing', on_delete=models.CASCADE)
    amount = models.DecimalField(max_digits=10, decimal_places=2, default=0)

    def top_up(self, amount):
        """Пополнение счета, если счет успешно пополнен, то вернется True"""
        if amount > 0:
            self.amount += amount
            self.save()
            return True
    ...
    def withdraw(self, amount):
        """Снятие денег со счета, если деньги снялись успешно, то вернется True"""
